...
void main()
{
    ...
    // pool bookkeeping (mirrors the contact-connectivity discipline).
    {
        ...
        Check( feq( s0->u.distanceJoint.length, 3.0 ) );              // rigid length stored
        Check( s0->u.distanceJoint.enableSpring == false );
    ...
    // body spins up to ~motorSpeed within a few steps.
    {
        ...
        jsim->u.revoluteJoint.enableMotor = true;
        jsim->u.revoluteJoint.motorSpeed = 2.0;
        jsim->u.revoluteJoint.maxMotorTorque = 100.0;
    ...
    // lower angle limit at -0.5 rad, resting there instead of hanging at ~-1.57.
    {
        ...
        b2JointSim* jsim = b2GetJointSim( &world, &world.joints.data[ jid ] );
        jsim->u.revoluteJoint.enableLimit = true;
        jsim->u.revoluteJoint.lowerAngle = -0.5;
        jsim->u.revoluteJoint.upperAngle = 0.5;
    ...
    // instead of falling forever.
    {
        ...
        b2JointSim* jsim = b2GetJointSim( &world, &world.joints.data[ jid ] );
        jsim->u.prismaticJoint.enableLimit = true;
        jsim->u.prismaticJoint.lowerTranslation = -1.0;
        jsim->u.prismaticJoint.upperTranslation = 3.0;   // stop 3 below the anchor
    ...
    // the body slides right at ~3 m/s.
    {
        ...
        b2JointSim* jsim = b2GetJointSim( &world, &world.joints.data[ jid ] );
        jsim->u.prismaticJoint.enableMotor = true;
        jsim->u.prismaticJoint.motorSpeed = 3.0;
        jsim->u.prismaticJoint.maxMotorForce = 100.0;
    ...
    // that free spin is the wheel-vs-slider discriminator).
    {
        ...
        b2JointSim* jsim = b2GetJointSim( &world, &world.joints.data[ jid ] );
        jsim->u.wheelJoint.enableLimit = true;
        jsim->u.wheelJoint.lowerTranslation = -1.0;
        jsim->u.wheelJoint.upperTranslation = 2.0;   // rest 2 below the anchor (y=8)
        jsim->u.wheelJoint.enableMotor = true;
        jsim->u.wheelJoint.motorSpeed = 2.0;
        jsim->u.wheelJoint.maxMotorTorque = 100.0;
    ...
    // flows through to the sim + solve (the box spins up to the def's motorSpeed).
    {
        ...
        Check( js->u.revoluteJoint.enableMotor == true );
        Check( feq( js->u.revoluteJoint.motorSpeed, 2.0 ) );
    ...
    // revolute motor but via the motor-joint solver's angular-velocity block.
    {
        ...
        b2JointSim* jsim = b2GetJointSim( &world, &world.joints.data[ jid ] );
        jsim->u.motorJoint.angularVelocity = 2.0;
        jsim->u.motorJoint.maxVelocityTorque = 100.0;
    ...
    // (3,0), capped by maxVelocityForce. With no gravity the body reaches ~3 m/s +x.
    {
        ...
        Check( feq( js->u.motorJoint.maxVelocityForce, 100.0 ) );
