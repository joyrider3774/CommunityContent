...
Vircon32 runs C compiled by its own toolchain (`compile` v26.04.24), and the language it
...
- **No `union`, no ternary `?:`, no compound literals, no `#pragma once`, no variadics, no
  token-paste/stringize, no `#if`/`#elif`.**
...
`bin/<name>.v32`. The toolchain comes with Vircon32 DevTools
(`compile` / `assemble` / `packrom`, compiler **v26.04.24**).

Run a ROM in the emulator (it auto-powers-on):

```
Vircon32 bin/harness.v32
```
...
- This port was made by [glebasos](https://github.com/glebasos), with the help of Claude AI.
