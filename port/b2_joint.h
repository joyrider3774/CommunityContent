...
//   Payloads live in the named union b2JointPayload (sim->u), overlaid like
//   upstream (converted 2026-07-19 after p11's correction; anonymous unions
//   remain unsupported, hence the `u.` member upstream doesn't have).
...
// Overlaid per-type joint payload, mirroring upstream's union in b2JointSim.
// DEVIATION: upstream's union is ANONYMOUS (unsupported form) -- this one is a
// named member `u`, so payload access carries a `u.` the upstream code lacks.
union b2JointPayload
{
...
struct b2JointSim
{
    ...
    // Per-type payload, overlaid like upstream's union (converted 2026-07-19 after
    // the p11 correction; shrinks the sim 212 -> ~63 words). The dialect has no
    // ANONYMOUS unions, so unlike upstream the union is a NAMED member: access is
    // sim->u.distanceJoint, not upstream's sim->distanceJoint. A joint sim is
    // always exactly ONE type; only the member matching sim->type is meaningful.
    b2JointPayload u;
};
